...
-- 001: Users, roles e admin guards  (consolidada: antigas 001 + 018)
--
-- - Helpers: wl_set_updated_at(), wl_is_admin(), wl_is_backend(), wl_is_member()
--   * wl_is_backend(): já na versão FINAL — aceita service_role E conexão
--     direta do n8n (sem JWT do PostgREST, fora dos roles anon/authenticated).
--   * wl_is_member(): membro da empresa OU backend confiável.
-- - CRUD de usuários via raw_user_meta_data (role + company_name='welmy')
-- - Apenas DOIS perfis: 'admin' e 'visualizacao'.
-- - Admin não exclui a si mesmo / não exclui o último admin.
-- =============================================
...
-- chamadas de backend (n8n via Postgres direto OU service_role do PostgREST)
...
-- membro da empresa (JWT) OU backend confiável (n8n)
...
-- DROP FUNCTION IF EXISTS wl_is_backend();
