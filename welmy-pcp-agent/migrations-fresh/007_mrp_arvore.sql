-- =============================================
-- Welmy — 007: MRP em ÁRVORE (antiga 015 — versão final com fallback de catálogo)
--
-- wl_mrp_arvore(plano) → JSONB pronto para a UI, respondendo às 3 perguntas do PCP:
--   1) Tempo TOTAL p/ o pedido ficar pronto = caminho crítico (MAIOR lead time
--      entre os itens pendentes; itens são providenciados em paralelo).
--   2) Tempo de CADA item = lead_time_dias (snapshot OU catálogo atual).
--   3) Quais já estão OK = necessidade_liquida <= 0 (estoque cobre).
--
-- Agrupa por SKU (sku_relacionado) quando existir; senão pelo grupo do item.
-- Mapeamento ERP: necessidade_bruta="Necessário", estoque_atual="Em estoque",
-- pedido_aberto_qtd="Em compra/produção", necessidade_liquida="Falta comprar".
-- Rode APÓS 005_suprimentos_rpc.sql
-- =============================================
