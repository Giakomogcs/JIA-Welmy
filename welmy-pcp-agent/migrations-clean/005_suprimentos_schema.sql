-- =============================================
-- Welmy — 005: Suprimentos & PCP (schema)
--
-- Modela o domínio do "Copiloto de Suprimentos e PCP":
--   * wl_fornecedor    — fornecedores / terceiros / montagem (lead time e atraso médio)
--   * wl_item          — componentes/SKUs (MP, comprado, fabricado, terceiro, embalagem)
--                        com lead time COMPOSTO (fornecedor + terceiro + montagem),
--                        estoque atual, estoque mínimo, ponto de pedido, curva A/B/C
--   * wl_pedido_compra — pedidos de compra em aberto (qtd + data prevista de chegada)
--   * wl_plano         — execução de um Relatório de Necessidades (upload/lote/mês)
--   * wl_necessidade   — linhas do relatório já analisadas (cobertura, risco, ação)
--   * wl_decision_log  — decisões da equipe (base de aprendizado)
--
-- Princípio (docx 5): motor de regras determinístico calcula; o LLM só explica.
...
CREATE TABLE IF NOT EXISTS wl_fornecedor (
  ...
  atraso_medio_dias    NUMERIC NOT NULL DEFAULT 0,  -- histórico de atraso (docx CR3)
  fornecedor_unico   BOOLEAN NOT NULL DEFAULT FALSE, -- único para item crítico (risco extra)
  contato            TEXT,
  observacoes        TEXT,
...
-- grupo (docx 1.4): define a ação recomendada conforme a origem do componente
CREATE TABLE IF NOT EXISTS wl_item (
  ...
  curva              TEXT CHECK (curva IN ('A','B','C')),  -- recorte de prioridade (Curva A)
  fornecedor_id      UUID REFERENCES wl_fornecedor(id) ON DELETE SET NULL,
  -- lead time COMPOSTO por etapa (docx: leadtime de cada sku = fornecedor + terceiro + montagem)
  ...
  lead_time_padrao_dias NUMERIC,                      -- prazo nominal do sistema (pode estar defasado)
  lead_time_real_dias   NUMERIC,                      -- prazo médio real (histórico) — CAMPO CRÍTICO
  -- estoque
  estoque_atual      NUMERIC NOT NULL DEFAULT 0,
  estoque_minimo     NUMERIC NOT NULL DEFAULT 0,
  ponto_pedido       NUMERIC,                          -- gatilho de reposição (docx CR6)
  consumo_diario     NUMERIC,                          -- média de consumo (un/dia) p/ dias até ruptura
  ativo              BOOLEAN NOT NULL DEFAULT TRUE,
  created_at         TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at         TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Cada upload de relatório gera um wl_plano. Todas as peças do plano precisam
-- chegar até o dia 25 (data_limite). Horizonte padrão de fabricação: 15 dias.
CREATE TABLE IF NOT EXISTS wl_plano (
  ...
  horizonte_dias  INT NOT NULL DEFAULT 15,             -- janela de fabricação (média 2500 peças/15 dias)
  qtd_pecas_plano NUMERIC,                             -- volume previsto do plano
...
CREATE TABLE IF NOT EXISTS wl_necessidade (
  ...
  pedido_aberto_qtd  NUMERIC NOT NULL DEFAULT 0,
  pedido_aberto_data DATE,
  lead_time_dias     NUMERIC,
  data_necessidade   DATE,                              -- quando o item precisa estar disponível
  -- saídas calculadas (motor de regras determinístico)
  necessidade_liquida NUMERIC,                          -- bruta - estoque (positivo = falta)
  ...
  impacto_skus        INT NOT NULL DEFAULT 0,             -- nº de SKUs finais que dependem do item
  risco              TEXT NOT NULL DEFAULT 'dado_incompleto'
                     CHECK (risco IN ('alto','medio','baixo','dado_incompleto')),
  prioridade         INT NOT NULL DEFAULT 0,              -- score de ordenação (maior = mais urgente)
  acao_sugerida      TEXT,                                -- comprar/acompanhar/fabricar/acionar/validar/revisar
  justificativa      TEXT,                                -- regra (texto curto determinístico)
...
CREATE TABLE IF NOT EXISTS wl_decision_log (
  ...
  concorda        BOOLEAN,                                -- concorda com a sugestão do agente?
  motivo          TEXT,
