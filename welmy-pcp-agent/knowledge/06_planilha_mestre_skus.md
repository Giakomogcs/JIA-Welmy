...
|---|---|---|
| **ANALISE DE SKUS** | produtos finais (SKUs) | DESCRIÇÃO DO PRODUTO, CODIGO, CURVAS, MEDIA MENSAL, **CONSUMO DIARIA ESTIMADO** |
...
> **fornecedor** (cadastrado em `wl_fornecedor`).
...
Use para descobrir:
- **consumo diário** de um SKU (aba ANÁLISE DE SKUS);
...
(mesmo ID) — não é preciso subir arquivo. Isso acontece pelo botão
**Sincronizar planilha** e automaticamente antes de processar os PDFs dos
relatórios. (Se um arquivo for enviado ao `welmy-migrate-planilha`, ele primeiro
substitui o arquivo no Drive e depois lê as abas.) Ao sincronizar, o sistema:
1. **recria o catálogo** a partir de ANALISE DE SKUS + LEAD TIMES
   (`wl_replace_planilha_mestre`);
...
4. **remove** os itens `fabricado` (produtos finais + componentes fabricados) que
   saíram da planilha; itens comprados/MP/embalagem não são removidos aqui;
